...
void starid::FloatsIndexer::add_pair(double newfloat, int newndx) {
    std::pair<double, int> pair{newfloat, newndx};
    float_ndx.push_back(pair);
}

void starid::FloatsIndexer::sort() { std::sort(float_ndx.begin(), float_ndx.end()); }

std::vector<int> starid::FloatsIndexer::findndxs(double lofloat, double hifloat) {
