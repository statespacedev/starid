...
# # -DPYBIND11_FINDPYTHON=ON  # initial cmake options on macbook
# # -DPYBIND11_PYTHON_VERSION=3.10  # final cmake options on macbook
...
class Demo:
    ...
    def SETTLER(self, starndx):
        """generate a star image for starndx and give it to SETTLER. the resulting id from the identifier should be
        the same as starndx. """
        img = self.api.image_generator(starndx)
        result = self.api.SETTLER(img['pixels'])
        print('SETTLER, starndx %i, result %i' % (starndx, result))

    def NOMAD(self, starndx):
        """generate a star image for starndx and give it to NOMAD. the resulting id from the identifier should be the
        same as starndx. """
        img = self.api.image_generator(starndx)
        result = self.api.NOMAD(img['pixels'])
        print('NOMAD, starndx %i, result %i' % (starndx, result))
