...
matching the classic mnist character recognition data set."""
...
class Image:
    ...
    def __init__(self, pvecs, info, doyaw=True):
    ...
    def img_to_pvecs(self):
        """convert image pixels to star pointing vectors / unit vectors in the 'camera frame'. these arent the 'real'
        hi-res pvecs we had when first creating the image. they're lo-res 'approximations' and have effectively lost
        or fuzzed out info during the transforms to and back from 28 x 28 pixels. several 'real' pvecs can end up as
        a single pixel."""
        pvecs = []
        for axjndx in range(28):
            for axindx in range(28):
                if self.img[axjndx, axindx] > 0:
                    ...
                    pvecs.append([x, y, sqrt(1 - x**2 - y**2)])
        return np.asarray(pvecs)
