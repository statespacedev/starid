     ...
     +          LIWS=2*IB+10*NMAX+12)
      ...
      REAL A(LA),B(MMAX),C(NMAX),X(NMAX+MMAX),WS(LWS),
     +                 G(NMAX),Z(NMAX),BND(2,NMAX),CNTL(15),RINFO(40),
     +                 CS(NMAX+1)
      EXTERNAL MC59A,LA04A
...
C Sort the entries by columns
      ...
      ICNTL(5) = 6
      ICNTL(6) = 0
      CALL MC59A(ICNTL,N,M,IA,IRN,LA,JCN,LA,A,N+1,IP,LIWS,IWS,INFO)

C Scale the problem
      JOB = 0
      CALL LA04A(A,LA,IRN,IP,M,N,B,C,BND,KB,LB,JOB,CNTL,IX,JX,X,Z,G,
...
C Perform simplex iterations
      JOB = 1
      DO 20 ITER = 1,1000
        CALL LA04A(A,IA,IRN,IP,M,N,B,C,BND,KB,LB,JOB,CNTL,IX,JX,X,Z,G,
     +              RINFO,WS,LWS,IWS,LIWS)
