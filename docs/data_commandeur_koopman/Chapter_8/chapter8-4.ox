/* File chapter8-4.ox
   ...
   illustration of filtered level, Kalman filter,
   and prediction errors.
   Ox/SsfPack code for Chapter 8, Section 4, of:
   ****************************************************************
...
DrawComponents(const mY)
{
	...
	print("\nmKF[][0:4]", mKF[][0:4]);
	mpe  = mKF[0][1:s_cT-1]; 
	mpev = ones(1,s_cT-1)./mKF[2][1:s_cT-1];
	print("\nmpev",mpev);
	...
	if (s_iLvlVar != -1)
	{
		//draw smoothed and filtered level
		DrawTMatrix(0, mks[0][], {"smoothed level"}, 1970, 1, 1, 0, 2);
		DrawTMatrix(0, mkf[0][1:31], {"filtered level"}, 1971, 1, 1, 0, 5);
		SaveDrawWindow("chapter8-4-1.eps");
		CloseDrawWindow();
		//draw prediction errors and their variance
		DrawTMatrix(0, mpe, {"prediction errors"}, 1971, 1, 1, 0, 6);
		DrawTMatrix(1, mpev, {"prediction error variance"}, 1971, 1, 1, 0, 6);
		SaveDrawWindow("chapter8-4-3.eps");
		CloseDrawWindow();
		//draw part of filtered level 1978-1983
		DrawTMatrix(0, mY[0][8:13], {"log fatalities Norway"}, 1978, 1, 1, 0, 1);
		DrawTMatrix(0, mkf[0][8:13], {"filtered level"}, 1978, 1, 1, 0, 5);
		DrawLine(0, 1980, mkf[0][10], 1981, mkf[0][10], 2);
		DrawText(0, "$a_{t}$", 1980, mkf[0][10]-0.01, -1, -1);
		DrawText(0, "$y_{t}$", 1980, mY[][10]-0.01, -1, -1);
		DrawText(0, "$y_{t}-a_{t}$", 1980, (mkf[0][10]+mY[][10])/2, -1, -1);
		DrawLine(0, 1980, mkf[0][10], 1980, mY[][10], 6);
		DrawLine(0, 1981, mkf[0][10], 1981, mkf[0][11], 6);
		DrawText(0, "$K_t(y_t-a_t)$", 1981, (mkf[0][10]+mkf[0][11])/2, -1, -1);
		DrawLine(0, 1981, mkf[0][11], 1982, mkf[0][11], 2);
		DrawText(0, "$a_{t+1}$", 1981, mkf[0][11]-0.01, -1, -1);
		DrawText(0, "$y_{t+1}$", 1981, mY[][11]+0.01, -1, -1);
		DrawText(0, "$y_{t+1}-a_{t+1}$", 1981, (mkf[0][11]+mY[][11])/2, -1, -1);
		DrawLine(0, 1981, mkf[0][11], 1981, mY[0][11], 6);
		DrawLine(0, 1982, mkf[0][11], 1982, mkf[0][12], 6);
		DrawText(0, "$K_{t+1}(y_{t+1}-a_{t+1})$", 1982, (mkf[0][11]+mkf[0][12])/2, -1, -1);
		DrawText(0, "$a_{t+2}$", 1982, mkf[0][12]-0.01, -1, -1);
		SaveDrawWindow("chapter8-4-2.eps");
		CloseDrawWindow();
	}
	else
	{
