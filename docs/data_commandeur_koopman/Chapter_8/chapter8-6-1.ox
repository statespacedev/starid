/* File chapter8-6-1.ox
   Local level model
   for log Norwegian fatalities: forecasting.
...
//	print("\nch = ",ch);
	...
	//compute filtered level and filtered estimation error variance
	decl mme;
	mp = SsfMomentEstEx(ST_PRED, &mme, s_mY, mphi, momega, msigma);
	print("\nmme[][0:4]",mme[][0:4]);
	
	// plot filtered level plus forecasts plus confidence intervals
	SetDraw(SET_COLORMODEL, 3);
	DrawTMatrix(0, mY, {"log fatalities in Norway"}, 1970, 1, 1, 0, 1);
	DrawTMatrix(0, mme[0][1:s_cT-1]~mf[cst][], {"filtered level and forecasts"},
								1971, 1, 1, 0, 2);
	DrawZ(sqrt(mme[2][1:s_cT-1]~mf[cst+csy][]), "", ZMODE_BAND, 1.64, 5);
	SaveDrawWindow("chapter8-6-1.eps");
}
