/* File chapter6.ox
   Level model with intervention variable SEATBELT LAW
   for log UK drivers KSI.
   Ox/SsfPack code for Chapter 6 of:
   ****************************************************************
...
LogLikStsm(const vY, const pdLik, const pdVar)
{
	...
	// add intervention variable SEATBELT LAW to local level model
	mjphi = <>;
...
DrawComponents(const mY)
{
	...
	if (s_iLvlVar != -1)
	{
		DrawTMatrix(0, mks[cst][], {"stochastic level + lambda*(SEATBELT LAW)"}, 1969, 1, 12, 0, 2);
		DrawTMatrix(1, mY - mks[cst][], {"irregular"}, 1969, 1, 12, 0, 6);
	}
	else
	{
		DrawTMatrix(0, mks[cst][], {"deterministic level + lambda*(SEATBELT LAW)"}, 1969, 1, 12, 0, 2);
		DrawXMatrix(1, mY, {"log UK drivers KSI against SEATBELT LAW"}, s_mX, {""}, 1, 1);   
		DrawXMatrix(1, mks[cst][], {"deterministic level + lambda*(SEATBELT LAW)"}, s_mX, {""}, 0, 2);   
		DrawTMatrix(2, mY - mks[cst][], {"irregular"}, 1969, 1, 12, 0, 6);
	}
	SaveDrawWindow("chapter6.eps");
...
main()
{
	...
	//construct intervention variable SEATBELT LAW
	decl cp0 = 169, cp1 = 23;
	s_mX = constant(0, 1, cp0) ~ constant(1, 1, cp1);
