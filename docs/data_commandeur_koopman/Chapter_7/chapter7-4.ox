/* File chapter7-4.ox
   ...
   with pulse variables
   for quarterly UK inflation prices.
   Ox/SsfPack code for Chapter 7, Section 4, of:
   ****************************************************************
...
SetStsmModel(const vP)
{
	...
	if ( (s_iLvlVar != -1) && (s_iSeaVar != -1))
		...
		s_vVarCmp =    0     |   0   | vr[0] | constant(vr[1],3,1) |  vr[2];
	else if ((s_iLvlVar != -1) && (s_iSeaVar == -1))
		s_vVarCmp =    0     |   0   | vr[0] | constant(0,11,1)     |  vr[1];
	else if ((s_iLvlVar == -1) && (s_iSeaVar != -1))
		s_vVarCmp =    0     |   0   |   0   | constant(vr[0],3,1) |  vr[1];
	else
		s_vVarCmp =    0     |   0   |   0   | constant(0,3,1)     |  vr[0];
}
LogLikStsm(const vY, const pdLik, const pdVar)
{
	...
	// add pulse variables to state space model
	mjphi = <>;
...
LogLikScoStsm(const vY, const pdLik, const pvSco)
{
	...
	{
		pvSco[0][0] = sumc(vs[3:5]);
		pvSco[0][1] = vs[6];
    	pvSco[0][] /= s_cT;			// scale score vector by sample size
	}
	else
	{
		pvSco[0] = vs[6] /s_cT;	// scale score vector by sample size
	}
...
DrawComponents(const mY)
{
	...
	cst = columns(mphi);
	cres = s_cT - cst;
	msofe = mKfilter[0][] .* sqrt(mKfilter[7][]);
	...
	mauxres1 = mdisturb[2][] ./ sqrt(mdisturb[9][]);
	mauxres2 = mdisturb[6][] ./ sqrt(mdisturb[13][]);
	...
	{
		DrawTMatrix(0, mtrend,
		{"stochastic level + pulse variables"},
								1950, 1, 4, 0, 2);
	...
	{
		DrawTMatrix(0, mtrend,
		{"deterministic level + pulse variables"},
								1950, 1, 4, 0, 2);
		DrawTMatrix(1, mseas, {"stochastic seasonal"},
								1950, 1, 4, 0, 3);
	}
...
main()
{
	...
	// set up pulse intervention variables for 1975-2 and 1979-3
	s_mX = (constant(0,1,101) ~ 1 ~ constant(0,1,106)) |
					(constant(0,1,118) ~ 1 ~ constant(0,1,89));
