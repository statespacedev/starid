/* File chapter4-1&4-2&4-3.ox
   Level and trigonometric seasonal model
   for log UK drivers KSI.
   Ox/SsfPack code for Chapter 4, Sections 1-3, of:
   ****************************************************************
...
SetStsmModel(const vP)
{
	...
	s_mStsm = <	CMP_LEVEL,     0.5,  0, 0;
    		   	CMP_SEAS_TRIG, 0.4, 12, 0;	// 12 for monthly data
				CMP_IRREG,       1,  0, 0>;
	...
	if ( (s_iLvlVar != -1) && (s_iSeaVar != -1))
		...
		s_vVarCmp =	vr[0]  | constant(vr[1],11,1) |   vr[2];
	else if ((s_iLvlVar != -1) && (s_iSeaVar == -1))
		s_vVarCmp =	 vr[0] | constant(0,11,1)     |   vr[1];
	else if ((s_iLvlVar == -1) && (s_iSeaVar != -1))
		s_vVarCmp =	   0   | constant(vr[0],11,1) |   vr[1];
	else
		s_vVarCmp =    0   | constant(0,11,1)     |   vr[0];
...
DrawComponents(const mY)
{
	...
	cst = columns(mphi);
	cres = s_cT - cst;
	msofe = mKfilter[0][] .* sqrt(mKfilter[13][]);
	...
	vseas = mks[1][]+mks[3][]+mks[5][]+mks[7][]+mks[9][]+mks[11][];  
	print("\nmks[][0:4]", mks[][0:4]);

	// compute smoothed disturbances
	SsfMomentEstEx(DS_SMO, &mdisturb, mY, mphi, momega, msigma);
	// compute auxiliary residuals
	mauxres1 = mdisturb[0][] ./ sqrt(mdisturb[13][]);
	mauxres2 = mdisturb[12][] ./ sqrt(mdisturb[25][]);

	SetDraw(SET_COLORMODEL, 3);
	DrawTMatrix(0, mY, {"log UK drivers KSI"}, 1969, 1, 12, 0, 1);
	if ( (s_iLvlVar != -1) && (s_iSeaVar != -1))
	{
		DrawTMatrix(0, mks[0][], {"stochastic level"}, 1969, 1, 12, 0, 2);
		DrawTMatrix(1, vseas, {"stochastic seasonal"}, 1969, 1, 12, 0, 3);
	}
	else if ((s_iLvlVar != -1) && (s_iSeaVar == -1))
	{
		DrawTMatrix(0, mks[0][], {"stochastic level"}, 1969, 1, 12, 0, 2);
		DrawTMatrix(1, vseas, {"deterministic seasonal"}, 1969, 1, 12, 0, 3);
	}
	else if ((s_iLvlVar == -1) && (s_iSeaVar != -1))
	{
		DrawTMatrix(0, mks[0][], {"deterministic level"}, 1969, 1, 12, 0, 2);
		DrawTMatrix(1, vseas, {"stochastic seasonal"}, 1969, 1, 12, 0, 3);
	}
	else
	{
	...
	DrawTMatrix(2, vseas[][0:11], {"seasonal 1969"}, 1969, 1, 12, 0, 3);
	DrawTMatrix(3, mY - mks[cst][], {"irregular"}, 1969, 1, 12, 0, 6);
	SaveDrawWindow("chapter4-1&4-2&4-3.eps");
	CloseDrawWindow();
	...
	DrawLine(1, 1969, -2, 1985, -2, 4);
	SaveDrawWindow("chapter4-1&4-2&4-3auxiliary.eps");
}
