/* File chapter3-4.ox
   ...
   for log Finnish fatalities.
   Ox/SsfPack code for Chapter 3, Section 4, of:
   ****************************************************************
...
DrawComponents(const mY)
{
	...
	DrawTMatrix(0, mY, {"log fatalities Finland"}, 1970, 1, 1, 0, 1);
	if ( (s_iLvlVar != -1) && (s_iSloVar != -1))
	{
		DrawTMatrix(0, mks[cst][], {"stochastic level and slope"}, 1970, 1, 1, 0, 2);
		DrawTMatrix(1, mks[cst-1][], {"stochastic slope"}, 1970, 1, 1, 0, 2);
	}
	else if ((s_iLvlVar != -1) && (s_iSloVar == -1))
	{
		DrawTMatrix(0, mks[cst][], {"stochastic level, deterministic slope"}, 1970, 1, 1, 0, 2);
		DrawTMatrix(1, mks[cst-1][], {"deterministic slope"}, 1970, 1, 1, 0, 2);
	}
	else if ((s_iLvlVar == -1) && (s_iSloVar != -1))
	{
		DrawTMatrix(0, mks[cst][], {"deterministic level, stochastic slope"}, 1970, 1, 1, 0, 2);
		DrawTMatrix(1, mks[cst-1][], {"stochastic slope"}, 1970, 1, 1, 0, 2);
	}
	else
	{
		DrawTMatrix(0, mks[cst][], {"deterministic level and slope"}, 1970, 1, 1, 0, 2);
		DrawTMatrix(1, mks[cst-1][], {"deterministic slope"}, 1970, 1, 1, 0, 2);
	}
	DrawTMatrix(2, mY - mks[cst][], {"irregular"}, 1970, 1, 1, 0, 6);
	SaveDrawWindow("chapter3-4.eps");
}
