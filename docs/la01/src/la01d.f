...
C       LA01FD removed from EXTERNAL statement.
...
C
C
C    LA01BD - A ROUTINE TO SOLVE THE LINEAR PROGRAMMING PROBLEM.
C      IT CALLS LA01CD TO SOLVE THE PROBLEM, THE MAIN FUNCTION
C      OF LA01BD IS TO SEGMENT THE WORKSPACE PROVIDED BY THE
C      CALLER.
C
      SUBROUTINE LA01BD(N,M,L,A,B,C,X,F,IA,IPRINT,IND,WK,IER)
C
C    WK   REAL ARRAY((M+1)*(M+3)); (*:LA01CD); WORKSPACE PROVIDED
C         BY THE CALLER.  LA01BD DIVIDES IT INTO SEGMENTS AND
C         PASSES THEM TO LA01CD.
C
C    ALL THE OTHER ARGUMENTS ARE DESCRIBED IN LA01CD.
C
...
C     ..
C     .. Array Arguments ..
      DOUBLE PRECISION A(IA,*),B(*),C(*),WK(*),X(*)
      INTEGER IND(*)
C     ..
C     .. Local Scalars ..
      INTEGER IU,IV,IW,M1
...
C      CALL LP ROUTINE
C
      CALL LA01CD(N,M,L,A,B,C,X,F,IA,IPRINT,IND,WK(IV),IER,WK(IU),M1,
     +            WK(IW))
C
C      RETURN TO CALLER
C
...
C    LA01CD - A ROUTINE TO SOLVE THE LINEAR PROGRAMMING PROBLEM.
...
C
      SUBROUTINE LA01CD(N,M,L,A,B,C,X,F,IA,IPRINT,IND,V,IER,U,IU,W)
C
...
C    X      REAL ARRAY(N); (*:LA01CD); THE SOLUTION.
C    F      REAL SCALAR; (*:LA01CD); VALUE OF OBJECTIVE FUNCTION.
...
C    IND    INTEGER ARRAY(M+1); (USER:LA01CD); BASIS INDICATOR.
C           THE USER MAY FORCE THE INITIAL BASIS BY SETTING NBASIS
C           (IN COMMON LA01DD) AND THE 1ST NBASIS ELEMENTS OF IND.
C           ON RETURN IND WILL HOLD THE FINAL BASIS.
C    V      REAL ARRAY(M+1); (*:LA01CD); WORKSPACE, CONTAINS THE
C           BASIC MARGINAL COSTS ON RETURN.
C    IER    INTEGER SCALAR; (*:LA01CD); ERROR FLAG,
C             IER=0  SUCCESS
...
C             IER=4  LA01CD FAILED TO FIND M INDEPENDENT COLS
C             IER=5  SOLUTION UNBOUNDED
C             IER=6  INVALID ARGUMENTS (N,M,L)
C    U      REAL ARRAY(M+1 BY M+1); (*:LA01CD); TRANSPOSE OF BASIS
C           INVERSE.
C    IU     INTEGER SCALAR; (USER:*); 1ST DIMENSION OF U
C    W      REAL ARRAY(M+1); (*:LA01CD); WORKSPACE, LP SOLUTION
C
...
C     ..
C     .. Array Arguments ..
      DOUBLE PRECISION A(IA,*),B(*),C(*),U(IU,*),V(*),W(*),X(*)
      INTEGER IND(*)
C     ..
C     .. Local Scalars ..
      DOUBLE PRECISION AMAX,BIG,CINDJ,REDUCE,RNDERR,RVIL,RWID,SCP,SMALL,
     +                 SUM,THETA,VI1,VI2,WK1,WK2,Y
...
C     ..
C     .. Common blocks ..
      COMMON /LA01DD/EPS,MAXINV,NBASIS,IP,ISP,TOL
      COMMON /LA01GD/NAME
...
C      TEST ARGUMENT VALUES
      RNDERR = FD05AD(1)
      BIG = FD05AD(5)*0.1D0
      IF (TOL.LT.ZERO) TOL = FD05AD(1)**.666667D0
      IF (N.LE.0) GO TO 540
      IF (M.LE.0) GO TO 540
      IF (L.GT.M) GO TO 540
...
C******************************************************
C
C      LA01ED BUILDS THE INVERSE A COLUMN AT A TIME PUTTING
...
C
C
      CALL LA01ED(M,N,L,A,IA,NBASIS,IND,U,IU,IE)
C
C      TEST FOR SUCCESS
...
C      BASIS COL WHICH HAS MAX(DABS(W(I))) TO ENSURE STABILITY
        IF (DABS(W(I)).LE.AMAX) GO TO 35
        AMAX = DABS(W(I))
        ID = I
   35 CONTINUE
C
...
C
C      RE-INVERT BASIS
  320 CALL LA01ED(M,N,L,A,IA,NB,IND,U,IU,IE)
C
C      TEST FOR SUCCESS
...
C      COLUMN FROM SLACKS
  380   V(I) = U(K-N,M1)
  390   IF (DABS(V(I)).LE.EPS) GO TO 400
        EPS = DABS(V(I))
  400 CONTINUE
      V(ID) = W(ID)
...
C*****************************************************
C
C      INITIALIZE THE COMMON BLOCK LA01DD
C
C*****************************************************
C
      BLOCK DATA LA01FD
...
C     ..
C     .. Data statements ..
      DATA MAXINV/1/,IP/6/,NBASIS/0/,ISP/6/
      DATA EPS/0D0/,TOL/-1.0D0/
      DATA NAME/'  LA01BD'/
C     ..
C     .. Executable Statements ..
C
C      THE USER MAY ALTER MAXINV, IP, TOL, NBASIS AND NAME.
C      LA01CD ALWAYS SETS EPS.
C
C    EPS    REAL SCALAR; (*:LA01CD); SET TO THE MAGNITUDE
...
C
      END
C    LA01ED - GIVEN AN LP TABLEAU CONSISTING OF AN M BY N MATRIX
C      A AUGUMENTED BY THE FIRST L COLUMNS OF THE IDENTITY MATRIX,
C      LA01ED SELECTS M COLUMNS FROM THE TABLEAU THAT FORM A
...
C
C@PROCESS DIRECTIVE('IBMD')
      SUBROUTINE LA01ED(M,N,L,A,IA,NBASIS,IND,U,IU,IEC)
C
...
C    IND    INTEGER ARRAY(M+1); (USER:LA01ED); ON ENTRY GIVES THE
...
C    U      REAL ARRAY(M+1 BY M+1); (*:LA01ED); WILL CONTAIN THE TRANSPO
C           OF THE BASIS INVERSE.  LA01ED ALSO SETS THE (M+1)TH ROW TO
C           ZEROS AND U(M+1,M+1) TO ONE, THE REST OF THE (M+1)TH
C           COL IS USED AS WORKSPACE.
C    IU     INTEGER SCALAR; (USER:*); 1ST DIMENSION OF U.
C    IEC    INTEGER SCALAR; (*:LA01ED); ERROR CODE, IT WILL BE
C           SET TO -1 ON A SUCCESSFUL INVERSION AND TO THE
C           NUMBER OF INDEPENDENT COLUMNS IF LA01ED IS UNABLE TO
...
C     ..
C     .. Array Arguments ..
      DOUBLE PRECISION A(IA,*),U(IU,*)
      INTEGER IND(*)
C     ..
C     .. Local Scalars ..
      DOUBLE PRECISION AMAX,RUIM1,SCP,SUM,UKM1,UKM2
...
C      HAS  MAX(DABS(ITH ROW OF INVERSE  INTO  COLUMN))
        JK = 0
        AMAX = ZERO
        DO 70 KL = 1,NN
          K = KL
...
C      COLUMN FROM A
   ...
   55     CONTINUE
   60     IF (DABS(SUM).LE.AMAX) GO TO 70
          JK = KL
          AMAX = DABS(SUM)
   70   CONTINUE
C
...
C
  180 RETURN
C
C      LA01ED UNABLE TO FIND M INDEPENDENT COLS
C
  190 IEC = I - 1
      GO TO 180
