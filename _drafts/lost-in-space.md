...
---
the core of the lost in space problem is the total absence of sensor pointing knowledge. in practical situations, there's usually at least some foreknowledge of the sensor's attitude. the lost in space problem is the extreme limiting case where sensor attitude uncertainty is infinite.

the terms 'pointing' and 'attitude' are used interchangeably here. 'yaw' around the pointing unit-vector is always included, and pointing is a rotation matrix, rotation vector, euler vector, quaternion, etc. the unknown yaw is what makes the overall pattern recognition problem challenging. there's no 'up and down', no 'gravity'. this isn't 'normal photo recognition', it's 'randomly rotated photo recognition'.
