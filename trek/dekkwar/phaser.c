...
/***           noch das letzte PH-Kommando ausgefuehrt  ***/
/***           wurde wegen Shared Memory                ***/
/*** 14.02.03: String in send_msg u. sort_msg auf 33 begr.*/
...
/*** 09.04.03: Treffer auf HW, hat bislang gefehlt      ***/
/**********************************************************/
...
/*** 01.04.08: Radarlee bei PH c ?                      ***/
/*** 02.04.08: BLACK_HOLE getroffen: Meldung und Ende   ***/
/*** 03.04.08: Fehler aneu bei CLOUD-Abfrage beseitigt  ***/
...
/*** 03.12.09: Einige Abfragen verschoben               ***/
/*** 30.12.09: seed() neu; ran() mit seed ueberarbeitet ***/
/*** 02.01.10: Umstellung auf init_expl()               ***/
/*** 23.01.10: Keine Explosion bei Romulaner mehr; das  ***/
...
int main(argc,argv)
/***************************************************************************/
/*** phaser  : Feuern des Phasers                                        ***/
/***         Aufruf in dwmain: PH C x n oder PH X Y n                    ***/
/***************************************************************************/
    int argc;
    char *argv[];
...
int xneu,yneu,xstart,ystart;
int hit_energy,ph_en,cloud_z;
char cm[2],aneu,comp,target,str1[MSG_LENGTH],str2[8];
float rxdiff,rydiff,rnd;
long t1,t2;
...
if (m_p->ship[vflag].hood==ON)
   {
   sort_msg("PH: Hood must be off!");
   goto start;
   }

if (m_p->ship[vflag].dam[4] > 300)
   {
   sort_msg("PH: Phaser out of order!");
   goto start;
   }

if (m_p->ship[vflag].dam[4] > 200)
   {
   sort_msg("PH: Phaser damaged, delay");
   usleep(1500000);
   }
/* b) */
if (sscanf(m_p->ph_cmd[vflag],"%s %c %c%d",cm,&comp,&target,&ph_en) == 4)
   {
   if (comp != 'C' && comp != 'c') 
      {
      if (sscanf(m_p->ph_cmd[vflag],"%s%d%d%d",cm,&xziel,&yziel,&ph_en) != 4)
         {
         sort_msg("     *** BAD COMMAND ***     ");
         goto start;
      	 }
      else  /* Ueberpruefung ob Koordinaten moeglich sind (0-(ARRAY_SIZE-1), diff>9) */
         {
      ...
      if (m_p->ship[vflag].dam[6]>250)
         {
         sort_msg("PH:    Computer damaged! ");
         sort_msg("PH:    Enter coordinates!");
         goto start;
         }
      zflag=det_target(target);
      if (zflag == (-1) || zflag == vflag) 
         {
         sort_msg("PH:     *** Wrong target ***");
         goto start;
         }
      if (m_p->ship[zflag].hood==ON) 
         {
         sort_msg("PH:   Target not detected!  ");
         goto start;
         }
      ...
      if (m_p->radarlee==ON)
	{
     ...
     sort_msg("PH: Computer locked in target!"); 
     }
   }
else  /* a) */
   {
   if (sscanf(m_p->ph_cmd[vflag],"%s%d%d%d",cm,&xziel,&yziel,&ph_en) != 3)
      {
      sort_msg("     *** BAD COMMAND ***     ");
      goto start;
      }
...
if (ph_en > max_ph[vflag] || ph_en<=0)
   {
   sprintf(str1,"PH: Wrong Phaser energy (1...%d)",max_ph[vflag]);
   sort_msg(str1);
   goto start;
   }
if (m_p->ship[vflag].ph_energy < ph_en)
   {
   sort_msg("PH: * Too less Phaser energy *");
   goto start;   
   }
...
if (m_p->ship[vflag].sv==ON)
  ...
  if (wegfrei(m_p->ship[vflag].pos[0],m_p->ship[vflag].pos[1],xziel,yziel)==1)
     {
     sort_msg("PH: Path to target obstructed!"); 
     sort_msg("    Phaser not fired!");
     goto start;
     }
  }
if ( (t1-t2) < 3 )
   {
   sort_msg("PH: Warning: high fire sequence!");
   sort_msg("    Damages of Phaser possible!!");
   if (incident((ph_en/5))==1)
      {
      rnd=ran();
      rnd=rnd*((float) ph_en);
      m_p->ship[vflag].dam[4]=m_p->ship[vflag].dam[4]+((int) rnd);
      }
   }
   
/* Phaserstrahl auf dem Weg */
hit_energy=ph_en;
sprintf(str1,"PH: Phaser fired to %03d %03d",xziel,yziel);
sort_msg(str1);         
...
/***************************************************************************/
/***   Berechnung des Weges vom Abschussort zum Ziel                     ***/
/***************************************************************************/
...
/***                         "Flug"                                      ***/
/***************************************************************************/
m_p->ship[vflag].ph_energy=m_p->ship[vflag].ph_energy-ph_en;
cloud_z=0;
for (i=1;i<=SHIP_RANGE;++i)
    {
    if (i>4) ph_en=ph_en-(ph_en/10);
    usleep(200000);
    /* Neue Position */
    ...
    if (aneu==CLOUD) 
       {
       ph_en=ph_en-(ph_en/10);
       cloud_z++;
       if (cloud_z==1) sort_msg("PH:Phaser beam weakened by cloud");
       hit(aneu,xneu,yneu,0); 
       }
    if (xneu<0 || yneu<0 || xneu>=ARRAY_SIZE || yneu>=ARRAY_SIZE)
       {
       sort_msg("PH: Phaser beam lost at the");
       sort_msg("    edge of the universe!");
       goto start;
       }
    /* Abfragen neue Position */
    if (aneu != SPACE && aneu!= CLOUD)
       {
    /* Meldung, was getroffen wurde */
       strcpy(str1,"PH: Phaser hit  ");
       str1[16]=aneu;
       str1[17]='\0';
       if (aneu == E_BASE)
          {
     ...
     /* Geisterbild */
       ...
       if (zflag!=(-1) && m_p->ship[zflag].pos[0]!=xneu)
          {
          aneu=m_p->a[xneu][yneu]=SPACE;
          sort_msg("PH: Ghost image, vanished!");
          }
     /* STAR,SAT,MINE,BLIND */
       ...
       if (aneu==BLACK_HOLE)
         {
	 sort_msg("PH: Phaser swallowed by Black Hole!");
         goto start;
         }
     ...
     /* Trefferwirkung fuer alle anderen Objekte */
      ...
      /* Homeworld getroffen */
       if (aneu==E_HOMEWORLD || aneu==F_HOMEWORLD)
          {
          sprintf(str1,"PH: Homew. got a %d units hit",hit_energy);
          sort_msg(str1);
      ...
      /* Planet getroffen */
       if (aneu==PLANET)
          {
          sprintf(str1,"PH: Planet got a %d units hit",hit_energy);
          sort_msg(str1);
          hit(PLANET,xneu,yneu,hit_energy); 
          goto start;
          }
       /* Fort getroffen */
       if (aneu==F_FORT || aneu==E_FORT)
          {
          sprintf(str1,"PH: Fort got a %d units hit",hit_energy);
          sort_msg(str1);
          hit(aneu,xneu,yneu,hit_energy); 
          goto start;
          }
       ...
       if (aneu==MISSILE)
          {
          sprintf(str1,"PH: Missile got a %d units hit",hit_energy);
          sort_msg(str1);
          hit(aneu,xneu,yneu,hit_energy); 
          goto start;
          }
       ...
       if (aneu==JUMPGATE)
          {
          sprintf(str1,"PH: Jumpgt. got a %d units hit",hit_energy);
          sort_msg(str1);
          hit(aneu,xneu,yneu,hit_energy); 
          goto start;
          }
       if (aneu==F_SCOUT || aneu==E_SCOUT)
          {
          sprintf(str1,"PH: Scout got a %d units hit",hit_energy);
          sort_msg(str1);
          hit(aneu,xneu,yneu,hit_energy); 
          goto start;
          }
       if (aneu==F_ROBOTSHIP || aneu==E_ROBOTSHIP)
          {
          sprintf(str1,"PH: Robotship got a %d units hit",hit_energy);
          sort_msg(str1);
          hit(aneu,xneu,yneu,hit_energy); 
          goto start;
          }
       ...
       if (aneu==F_BASE || aneu==E_BASE)
          {
          sprintf(str1,"PH: Base got a %d units hit",hit_energy);
          sort_msg(str1);
          hit(aneu,xneu,yneu,hit_energy);
          goto start;
          }
       ...
       if (aneu==HOOD)
          {
          sprintf(str1,"PH: Obj. got a %d units hit",hit_energy);
          sort_msg(str1);
          hit(HOOD,xneu,yneu,hit_energy); 
          goto start;
          }          
       ...
       if (zflag!=-1)
          {
          sprintf(str1,"PH: %c got a %d units hit",aneu,hit_energy);
          sort_msg(str1);
...
strcpy(str1,"PH: Phaser beam lost at ");
sprintf(str2,"%d %d",xneu,yneu);
strcat(str1,str2);
sort_msg(str1);
...
if (obj==JUMPGATE)
   ...
   if (m_p->jumpgate[nr].dam>=MAX_DAM_JG)
      ...
      init_expl(x,y,EXPL_TIME);
      sprintf(print_str,"Jumpgate at %d %d destroyed!",x,y);
      sort_msg(print_str);
      m_p->user_score[vflag]=m_p->user_score[vflag]+pntsd;
      m_p->name_score[vflag]=m_p->name_score[vflag]+pntsd;
      m_p->score[party[vflag]]=m_p->score[party[vflag]]+pntsd;
...
while (i<steps && cond==0)
    ...
    yneu=yalt+ (NINT( (((float) i)*rydiff) ));
    aneu=m_p->a[xneu][yneu];
    if (aneu!=SPACE && aneu!=EXPLOSION && aneu!=CLOUD && aneu!=BLIND) cond=1;
    i++;       
    } 
