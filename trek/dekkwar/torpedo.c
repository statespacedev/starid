...
/*** 10.02.03: Bei Treffer auf Homeworld nicht laenger  ***/
...
/*** 20.03.03: Neue Version, kein Dauerprozess mehr.    ***/
/***           anz_ttubes und used_ttubes eingefuehrt.  ***/
/*** 25.03.03: Dauer des Nachladens vom Schiffstyp abh. ***/
/*** 04.04.03: Pause beim Nachladen jetzt in dwmain.c   ***/
/*** 04.04.03: Unterprogramm eliminate_hw jetzt in libdw***/
/*** 11.04.03: m_p->torp[vflag]-- vorverlegt wegen des  ***/
/***           Problems mit negativer Torpedoanzahl.    ***/
/*** 11.04.03: Status Torpedo-Tubes hinzugefuegt        ***/
...
/**********************************************************/
/*** 11.02.08: Wenn T. auf EXPLOSION trifft, wird T.    ***/
...
/**********************************************************/
/*** 29.02.08: Neue Scout-Behandlung + hit ==> thit     ***/ 
...
/**********************************************************/
/*** 31.03.08: 9 durch SHIP_RANGE ersetzt               ***/
/*** 01.04.08: Radarlee bei TO c ?                      ***/
/*** 02.04.08: BLACK_HOLE getroffen: Meldung und Ende   ***/
/*** 03.04.08: SV-Flag eingebaut (wegfrei)              ***/
/**********************************************************/
...
/*** 03.12.09: Checks (Dam,Anz.T.,Hood) -> dwmain.c     ***/
/*** 04.12.09: Delay bei Schaden jetzt schadensabh.     ***/
/*** 12.12.09: Hauptschleife ueberarbeitet; Meldungen   ***/
...
/*** 20.12.09: Bufferoverflow Textausgabe beseitigt     ***/
/*** 24.12.09: In thit Abfrage auf EXPLOSION eingebaut; ***/
...
/*** 25.12.09: In thit umgestellt auf init_expl()       ***/
/*** 23.01.10: Keine Explosion des Romulaners mehr;     ***/
/***           erfolgt durch den Rom. selbst            ***/
/*** 24.01.10: Keine Explosion des Scouts mehr; erfolgt ***/
/***           ausschliesslich durch den Scout selbst   ***/
/*** 24.01.10: Doppeltes Scoring bei Scout-Zerstoerung  ***/
/***           beseitigt                                ***/
...
/***         Aufruf in dwmain: TO C x oder TO X Y                        ***/
/***************************************************************************/
    int argc;
    char *argv[];
...
int xneu,yneu,xalt,yalt,xstart,ystart;
int hit_energy;
char cm[2],aneu,aalt,comp,target,str1[MSG_LENGTH],str2[13];
float rxdiff,rydiff;
...
/***         a)  Koordinaten x y b) C plus Ziel                           ***/
/****************************************************************************/
zflag=xziel=yziel=(-1);
target=comp='?';

if (m_p->ship[vflag].dam[3] > 200)
   {
   sort_msg("TO: torpedo tubes damaged,delay");
   usleep(m_p->ship[vflag].dam[3]*m_p->ship[vflag].dam[3]*20);
   }

/* Schiff feuert: Kommandostring auf Logik ueberpruefen */
/* Erst computergesteuert, dann Koordinaten */ 
if (sscanf(m_p->to_cmd[vflag],"%s %c %c",cm,&comp,&target) == 3)
   {
   if (comp != 'C' && comp != 'c') 
      {
      if (sscanf(m_p->to_cmd[vflag],"%s%d%d",cm,&xziel,&yziel) != 3)
         {
         sort_msg("     *** BAD COMMAND ***     ");
         (void) shmdt(m_p);
         exit(0);
      	 }
      else  /* Ueberpruefung ob Koordinaten moeglich sind (0-(ARRAY_SIZE-1), diff>SHIP_RANGE) */
         {
         if (xziel<0 || xziel >(ARRAY_SIZE-1) || yziel<0 || yziel>(ARRAY_SIZE-1))
            {
            sort_msg("TO: *** Wrong coordinates ***");
            (void) shmdt(m_p);
            exit(0);
            }
         if (xziel == m_p->ship[vflag].pos[0] && yziel == m_p->ship[vflag].pos[1])
            {
            sort_msg("TO: ***    Our position   ***");
            (void) shmdt(m_p);
            exit(0);
            }
         if ( (IABS( (m_p->ship[vflag].pos[0]-xziel) ) ) > SHIP_RANGE 
                          || 
              (IABS( (m_p->ship[vflag].pos[1]-yziel) ) ) > SHIP_RANGE )
            {
            sort_msg("TO:    *** Out of range ***  ");
            (void) shmdt(m_p);
            exit(0);
            }
         }
      }   
   else	/* Nur Schiffe als Ziele moeglich */
      {
      if (m_p->ship[vflag].dam[6]>250)
         {
         sort_msg("TO:    Computer damaged! ");
         sort_msg("TO:    Enter coordinates!");
         (void) shmdt(m_p);
         exit(0);
         }
      zflag=det_target(target);
      if (zflag == (-1) || zflag == vflag) 
         {
         sort_msg("TO:     *** Wrong target ***");
         (void) shmdt(m_p);
         exit(0);
         }
      if (m_p->ship[zflag].hood==ON) 
         {
         sort_msg("TO:  Target not detected!   ");
         (void) shmdt(m_p);
         exit(0);
         }
      /*** Position des Ziels auf xziel,yziel setzen ***/
      xziel=m_p->ship[zflag].pos[0];
      yziel=m_p->ship[zflag].pos[1];
      /** Abfrage, ob Coordinaten o.k. ***/
      if (xziel<0 || xziel >= ARRAY_SIZE || yziel<0 || yziel>=ARRAY_SIZE)
         {
         sort_msg("TO: *** Wrong coordinates ***");
         (void) shmdt(m_p);
         exit(0);
      ...
      if (m_p->ship[zflag].dock == DOCKED)
         {
         sort_msg("TO: target not detected!");
         (void) shmdt(m_p);
         exit(0);
         }
     /*** Ziel sichtbar (Radarlee)? **/
     if (m_p->radarlee==ON)
       {
       if (behind(xziel,yziel,m_p->ship[vflag].pos[0],m_p->ship[vflag].pos[1],SHIP_RANGE)==1)
          {
          sort_msg("TO: target not detected! "); 
          (void) shmdt(m_p);
          exit(0);
     ...
     sort_msg("TO: Computer locked onto target!"); 
     }
   }
else /* Schiff feuert auf Koordinaten */
   {
   if (sscanf(m_p->to_cmd[vflag],"%s%d%d",cm,&xziel,&yziel) != 3)
      {
      sort_msg("     *** BAD COMMAND ***     ");
   ...
   else  /* Ueberpruefung ob Koordinaten moeglich sind (0-199, diff>9) */
      {
      if (xziel<0 || xziel >(ARRAY_SIZE-1) || yziel<0 || yziel>(ARRAY_SIZE-1))
         {
         sort_msg("TO: *** Wrong coordinates ***");
...
/** Coordinaten o.k., Ziel o.k. ***/

/* Weg frei (SV)? */
if (m_p->ship[vflag].sv==ON)
  {
  if (wegfrei(m_p->ship[vflag].pos[0],m_p->ship[vflag].pos[1],xziel,yziel)==1)
    {
    sort_msg("TO: Path to target obstructed!"); 
    sort_msg("    Torpedo not fired!");
    (void) shmdt(m_p);
    exit(0);
    }
  }

/* Torpedo wird gestartet */
m_p->ship[vflag].torp--;
/* TTube bestimmen */
nr_ttube=0;
if (level_of_ship[vflag]==DESTROYER)
   {
   nr_ttube=0;
   m_p->ship[vflag].status_ttubes[nr_ttube]=RELOADING;
   }
else if (level_of_ship[vflag]==CRUISER)
  {
  if (m_p->ship[vflag].status_ttubes[0]==LOADED)
    {
    nr_ttube=0;
    m_p->ship[vflag].status_ttubes[nr_ttube]=RELOADING;
    }
...
else if (level_of_ship[vflag]==BATTLESHIP)
  {
  if (m_p->ship[vflag].status_ttubes[0]==LOADED)
    {
    nr_ttube=0;
  ...
  else if (m_p->ship[vflag].status_ttubes[1]==LOADED)
    {
    nr_ttube=1;
    m_p->ship[vflag].status_ttubes[nr_ttube]=RELOADING;
    }
...
else if (level_of_ship[vflag]==CARRIER)
  {
  if (m_p->ship[vflag].status_ttubes[0]==LOADED)
    {
    nr_ttube=0;
  ...
  {
  sort_msg("TO: TTubeError!");
  }
/* Zeitpunkt des Starts fuer TT-Reload-Zeit; wird in Cockpit ueberwacht */
clock_gettime(CLOCK_REALTIME, &ntime);
m_p->ship[vflag].tt_lock_time[nr_ttube]=((long long)ntime.tv_sec*1000000000) + ntime.tv_nsec;

sprintf(str1,"TO: torpedo fired to %03d %03d",xziel,yziel);
sort_msg(str1);         

if (m_p->ship[vflag].shield==ON)
   {
   m_p->ship[vflag].main_energy=m_p->ship[vflag].main_energy-80;
   usleep(100000);
   }
else
   {
...
xstart=m_p->ship[vflag].pos[0];
ystart=m_p->ship[vflag].pos[1];
xsteps=xziel-xstart;
ysteps=yziel-ystart;
...
for (i=1;i<=SHIP_RANGE;++i)
   {
    ...
    if (aneu == BLIND && i==1) 
       {
       usleep(200000);             /* neu am 25.3.03: wenn schnell hinter-*/
       aneu=m_p->a[xneu][yneu];    /* einander Torpedos gefeuert werden,
                                      kann der letzte den vorletzten treffen */
       }
    if (xneu<0 || yneu<0 || xneu>=ARRAY_SIZE || yneu>=ARRAY_SIZE)
       {
       m_p->a[xalt][yalt]=aalt;       
       sort_msg("TO: torpedo lost at the edge");       
       sort_msg("    of the universe!");
       (void) shmdt(m_p);
       exit(0);
       }
    if (aneu == EXPLOSION)   
       {
       m_p->a[xalt][yalt]=aalt;
       sort_msg("Torpedo destroyed by explosion");
       (void) shmdt(m_p);
       exit(0);
       }
    ...
    if (aneu != SPACE)
       ...
       if (zflag>=0 && m_p->ship[zflag].pos[0]!=xneu)
          ...
          sort_msg("TO: Ghost image, vanished!");
          (void) shmdt(m_p);
          exit(0);
          }
       strcpy(str1,"TO: torpedo hit ");
       zflag=det_target(aneu);
       if (zflag >= 0)
	  {
          sprintf(str2,"%c at %03d %03d",ships[zflag][0],xneu,yneu);
          strcat(str1,str2);
          sort_msg(str1);
          sprintf(str1,"TO: %c got a %d units hit",ships[zflag][0],hit_energy);
          sort_msg(str1);
          } 
       else if (aneu == E_BASE)
          {
	  strcat(str1,"Imperial Base");
          sort_msg(str1);
          sprintf(str1,"     at %03d %03d!",xneu,yneu);
          sort_msg(str1);
          sprintf(str1,"TO: base got a %d units hit",hit_energy);
          sort_msg(str1);
          }
       else if (aneu == F_BASE)
          {
	  strcat(str1,"Federal Base");
          sort_msg(str1);
          sprintf(str1,"     at %03d %03d!",xneu,yneu);
          sort_msg(str1);
          sprintf(str1,"TO: base got a %d units hit",hit_energy);
          ...
          {
          strcat(str1,"Imp. Homeworld");\
          sort_msg(str1);
          sprintf(str1,"     at %03d %03d!",xneu,yneu);
          sort_msg(str1);
          sprintf(str1,"TO: Homew. got a %d units hit",hit_energy);
          sort_msg(str1);
          }
       else if (aneu == F_HOMEWORLD)
          {
          strcat(str1,"Fed. Homeworld");
          sort_msg(str1);
          sprintf(str1,"     at %03d %03d!",xneu,yneu);
          sort_msg(str1);
          sprintf(str1,"TO: Homew. got a %d units hit",hit_energy);
       ...
       else if (aneu == F_ROBOTSHIP)
          {
          strcat(str1,"Fed. Robotship");
          sort_msg(str1);
          sprintf(str1,"     at %03d %03d!",xneu,yneu);
          sort_msg(str1);
          sprintf(str1,"TO: r.ship got a %d units hit",hit_energy);
          sort_msg(str1);
          }
       else if (aneu == E_ROBOTSHIP)
          {
          strcat(str1,"Imp. Robotship");
          sort_msg(str1);
          sprintf(str1,"     at %03d %03d!",xneu,yneu);
          sort_msg(str1);
          sprintf(str1,"TO: r.ship got a %d units hit",hit_energy);
          ...
          {
          strcat(str1,"Imperial Fort");
          sort_msg(str1);
          sprintf(str1,"     at %03d %03d!",xneu,yneu);
          sort_msg(str1);
          sprintf(str1,"TO: fort got a %d units hit",hit_energy);
          sort_msg(str1);
          }
       else if (aneu == F_FORT)
          {
          strcat(str1,"Federal Fort");
          sort_msg(str1);
          sprintf(str1,"     at %03d %03d!",xneu,yneu);
          sort_msg(str1);
          sprintf(str1,"TO: fort got a %d units hit",hit_energy);
       ...
       else if (aneu == JUMPGATE)
          {
          strcat(str1,"Jumpgate");
          sort_msg(str1);
          sprintf(str1,"     at %03d %03d!",xneu,yneu);
          sort_msg(str1);
          sprintf(str1,"TO: j.g. got a %d units hit",hit_energy);
          sort_msg(str1);
          }
       else if (aneu == E_SCOUT)
          {
          strcat(str1,"Imperial Scout");
          sort_msg(str1);
          sprintf(str1,"     at %03d %03d!",xneu,yneu);
          sort_msg(str1);
          sprintf(str1,"TO: scout got a %d units hit",hit_energy);
          sort_msg(str1);
          }
       else if (aneu == F_SCOUT)
          {
          strcat(str1,"Federal Scout");
          sort_msg(str1);
          sprintf(str1,"     at %03d %03d!",xneu,yneu);
          sort_msg(str1);
          sprintf(str1,"TO: scout got a %d units hit",hit_energy);
          ...
          {
          sprintf(str1,"TO: torpedo hit star at %03d %03d",xneu,yneu);
          sort_msg(str1);
          }
       else if (aneu == MINE)
          {
          sprintf(str1,"TO: torpedo hit mine at %03d %03d",xneu,yneu);
          sort_msg(str1);
          }
       else if (aneu == HOOD)
          {
          sprintf(str1,"TO: torpedo hit UFO at %03d %03d",xneu,yneu);
          sort_msg(str1);
          sprintf(str1,"TO: UFO got a %d units hit",hit_energy);
          sort_msg(str1);
          }
       else if (aneu == PLANET)
          {
          ...
          sprintf(str1,"     at %03d %03d!",xneu,yneu);
          sort_msg(str1);
          sprintf(str1,"TO: planet got a %d units hit",hit_energy);
          sort_msg(str1);
          }
       else if (aneu == BLIND)
          {
          ...
          {
	  sort_msg("TO: torpedo swallowed by B.Hole");
	  (void) shmdt(m_p);
	  exit(0);
          }
       else if (aneu==CLOUD)
          {
          sort_msg("TO: torpedo hit gas cloud!");
          }
       else
	  {
	  sort_msg("TO: torpedo hit undef. object");
          }
       thit(aneu,xneu,yneu,hit_energy); 
       (void) shmdt(m_p);
       exit(0);
       }
    /* Umsetzen der Werte */
    m_p->a[xneu][yneu]=BLIND;
    if (aalt==ships[vflag][0] && (xalt!=m_p->ship[vflag].pos[0] || yalt!=m_p->ship[vflag].pos[1]) )
       {
...
strcpy(str1,"TO:  Torpedo lost at ");
sprintf(str2,"%03d %03d",xneu,yneu);
strcat(str1,str2);
sort_msg(str1);
...
int gauss1()
/***************************************************************************/
/*** Berechnet die Trefferenergie bei Torpedos                           ***/
/*** (Gaussverteilung um ave_torp_pow)                                   ***/
/***************************************************************************/
{
int x1,x2;
float fx,rnd;
...
x1=(ave_torp_pow[vflag]-(NINT( (((float) ave_torp_pow[vflag])*(1.0-fx)))));
x2=(ave_torp_pow[vflag]+(NINT( (((float) ave_torp_pow[vflag])*(1.0-fx)))));
if (rnd<0.5) 
   { 
   return(x1);
   }
